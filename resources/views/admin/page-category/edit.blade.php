        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    Chỉnh sửa danh mục trang
                </div>
                <div class="card-body">
                    @include('admin.includes.notification')
                    <form action="{{route('page-category.update', $category->id)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
  
                        @if (config('page.category_level') > 1)
                            <div class="mb-3">
                                <label>Danh mục cha</label>
                                <select class="form-control select2" name="parent_id">
                                    <option value="">- Không có -</option>
                                    @foreach($categories as $key => $item)
                                        @include('admin.page-category.includes.option', ['category' => $item, 'selected' => $category->parent_id])
                                    @endforeach
                                </select>
                            </div>
                        @endif
