@extends('layouts.app')

@section('content')
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="mb-3">
                <a href="{{ route('product.create') }}" class="btn btn-success">Thêm mới</a>
            </div>
            <div class="card mb-4">
                <div class="card-header d-flex justify-content-between">
                    Lọc sản phẩm
                </div>
                <div class="card-body">
                    <form action="{{ route('product.index') }}" method="GET">
                        <div class="row">
                            <div class="col-md-3">
                                <input type="text" class="form-control" name="keyword" placeholder="Nhập từ khóa" value="{{ request()->keyword ?? '' }}">
                            </div>
                            <div class="col-md-3">
                                <select name="category" class="form-control select2" style="width: 100%">
                                    <option value="">- Tất cả danh mục -</option>
                                    @foreach($categories as $category)
                                        @include('admin.product-category.includes.option', ['category' => $category, 'selected' => request()->category, 'full' => 1])
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-3">
                                <select name="brand" class="form-control select2" style="width: 100%">
                                    <option value="">- Tất cả thương hiệu -</option>
                                    @foreach($brands as $brand)
                                        <option value="{{ $brand->id }}" {{ request()->brand && request()->brand == $brand->id ? 'selected' : '' }}>{{ $brand->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-3">
                                <button type="submit" class="btn btn-primary">Tìm kiếm</button>
                                <a href="{{ route('product.index') }}" class="btn btn-success">Làm mới</a>
                            </div>
                        </div>
                    </form>
                </div>
            </div>
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    Danh sách sản phẩm
                    <small class="text-secondary">{{ "Hiển thị " .  $products->firstItem() . "-" . $products->lastItem() . " của " . $products ->total() . ' sản phẩm' }}</small>
                </div>
                <div class="card-body">
                    @include('admin.includes.notification')
                    @if ($products->count())
                    <div class="table-responsive">
                        <table class="table">
                            <thead>
                                <tr>
                                    <th scope="col">Hình ảnh</th>
                                    <th scope="col">Tên sản phẩm</th>
                                    <th scope="col">Tên danh mục</th>
                                    <th scope="col">Giá bán</th>
                                    <th scope="col">Khuyến mãi</th>
                                    <th scope="col" class="text-center">Trạng thái</th>
                                    <th scope="col">Ngày cập nhật</th>
                                    <th scope="col">Thao tác</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($products as $item)
                                    <tr data-id="{{ $item->id }}">
                                        <td>
                                            @if ($item->avatar()->count())
                                                <img src="{{ asset($item->avatar->image) }}" width="30" height="30">
                                            @else
                                                <img src="{{ asset('uploads/default.png') }}" width="30" height="30">
                                            @endif
                                        </td>
                                        <td>{{ $item->name }}</td>
                                        <td>{{ $item->category->name }}</td>
                                        <td><b>{{ number_format($item->price) }}</b></td>
                                        <td>{{ $item->discount }}</td>
                                        <td class="text-center">
                                            <div class="form-check d-inline-block form-switch">
                                                <input class="form-check-input status-switch" type="checkbox" {{ $item->status == 1 ? 'checked' : '' }}>
                                            </div>
                                        </td>
                                        <td>{{ $item->updated_at }}</td>
                                        <td>
                                            <a href="{{ route('product.edit', $item->id) }}" class="btn btn-primary btn-sm">Chỉnh sửa</a>
                                            <form action="{{ route('product.destroy', $item->id) }}" method="POST" class="d-inline-block">
                                                @csrf
                                                @method("DELETE")
                                                <button type="submit" class="btn btn-sm btn-danger btnDelete">Xóa</button>
                                            </form>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                        {{ $products->links() }}
                        @else
                            <p class="text-center">Không có kết quả nào.</p>
                        @endif
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
@endsection

@push('scripts')
<script>

    $('.status-switch').change(function(){
        let _this = $(this);
        let id =  _this.closest('tr').attr('data-id');
        let status = _this.is(':checked') ? 1 : 0;
        let data = [{'id': id, 'status': status }];
        $.ajax({
            url: '{{ route("product.massUpdate") }}',
            type: 'POST',
            data: JSON.stringify(data),
            beforeSend: function() {
                _this.hide();
                _this.closest('.form-check').addClass('ps-0');
                _this.after('<i class="fa-solid fa-spinner fa-spin"></i>');
            },
            success: function(res){
                if(res && res.success) {
                    console.log(res);
                }
            },
            complete: function() {
                _this.show();
                _this.closest('.form-check').removeClass('ps-0');
                _this.next('i').remove();
            },
        });
    });
</script>
@endpush