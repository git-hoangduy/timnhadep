        ...
        <div class="col-md-6">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="{{route('project-category.update', $category->id)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
  
                        @if (config('project.category_level') > 1)
                            <div class="mb-3">
                                <label>Danh mục cha</label>
                                <select class="form-control select2" name="parent_id">
                                    <option value="">- Không có -</option>
                                    @foreach($categories as $key => $item)
                                        @include('admin.project-category.includes.option', ['category' => $item, 'selected' => $category->parent_id])
                                    @endforeach
                                </select>
                            </div>
                        @endif
