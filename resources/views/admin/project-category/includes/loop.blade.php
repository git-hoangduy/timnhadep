...
<tr data-id="{{ $item->id }}">
    ...
    <td>
        <a href="{{ route('project-category.edit', $category->id) }}" class="btn btn-primary btn-sm">Chỉnh sửa</a>
        <form action="{{ route('project-category.destroy', $category->id) }}" method="POST" class="d-inline-block">
            @csrf
            @method("DELETE")
...
@if($category->children->count() > 0)
    @foreach($category->children as $item)
        @include('admin.project-category.includes.loop', ['category' => $item])
    @endforeach
@endif